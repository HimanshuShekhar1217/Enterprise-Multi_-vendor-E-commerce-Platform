...
public class ProductDataSeeder {
    ...
    @Bean
    CommandLineRunner seedProducts(ProductRepository products, UserRepository users, JdbcTemplate jdbcTemplate) {
        return args -> {
            jdbcTemplate.execute("ALTER TABLE products ADD COLUMN IF NOT EXISTS stock INTEGER NOT NULL DEFAULT 0");

            users.findByEmail("vendor@shopstack.com").ifPresent(vendor -> {
            ...
            seeds.forEach(seed -> {
                if (products.findByName(seed.name()).isEmpty()) {
