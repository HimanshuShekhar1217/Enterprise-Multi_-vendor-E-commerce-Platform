...
export default function StaffReturnedStock() {
  ...
  useEffect(() => {
    const load = () => fetch("http://localhost:8080/api/admin/refunds", { headers: { Authorization: `Bearer ${sessionStorage.getItem("token") || localStorage.getItem("token")}` } }).then((response) => response.ok ? response.json() : []).then((records) => setItems(records.filter((item) => item.refundStatus === "REFUNDED"))).catch(() => setItems([])).finally(() => setLoading(false));
